...
# 楽天証券CTA: タイトル「楽天証券で新NISA口座を開設」 → DMM株に置換
RE_OLD_RAKUTEN_SHOKEN = re.compile(
    r'<div class="hb-cta-box[^"]*">\s*'
    r'<span class="hb-cta-label">[^<]*</span>\s*'
    r'<h3 class="hb-cta-title">楽天証券[^<]*</h3>'
    r'.*?</div>',
    re.DOTALL,
)

# 楽天デビットCTA: タイトル「楽天銀行デビット VISA」 → Nexus Card に置換
RE_OLD_RAKUTEN_DEBIT = re.compile(
    r'<div class="hb-cta-box[^"]*">\s*'
    r'<span class="hb-cta-label">[^<]*</span>\s*'
    r'<h3 class="hb-cta-title">楽天銀行デビット[^<]*</h3>'
    r'.*?</div>',
    re.DOTALL,
)

# マネーフォワードCTA: タイトル「マネーフォワードME」 → ココナラに置換
RE_OLD_MFW = re.compile(
    r'<div class="hb-cta-box[^"]*">\s*'
    r'<span class="hb-cta-label">[^<]*</span>\s*'
    r'<h3 class="hb-cta-title">マネーフォワード[^<]*</h3>'
    r'.*?</div>',
    re.DOTALL,
)
...
def fix_content(html):
    """3 種類の古いCTAボックスを新しいブランドのCTAに置換。戻り値: (新HTML, 置換数dict)"""
    counts = {"RAKUTEN_SHOKEN→DMM_KABU": 0, "RAKUTEN_DEBIT→NEXUS": 0, "MFW→COCONALA": 0}
    new = html

    new, n1 = RE_OLD_RAKUTEN_SHOKEN.subn(NEW_CTA_DMM_KABU, new)
    counts["RAKUTEN_SHOKEN→DMM_KABU"] = n1

    new, n2 = RE_OLD_RAKUTEN_DEBIT.subn(NEW_CTA_NEXUS, new)
    counts["RAKUTEN_DEBIT→NEXUS"] = n2

    new, n3 = RE_OLD_MFW.subn(NEW_CTA_COCONALA, new)
